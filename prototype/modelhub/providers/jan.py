"""Jan provider — connects to local Jan server (OpenAI-compatible)."""
...
logger = logging.getLogger("alfa.modelhub.jan")


class JanProvider(ModelProvider):
    """Provider for Jan local server (OpenAI-compatible API)."""
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self._base_url = self._config.get("base_url", "http://localhost:1337")
    ...
    def load(self) -> None:
        ...
        logger.info("Jan provider loaded: %d models", len(self._models))
    ...
    def _discover_models(self) -> None:
        try:
            ...
            logger.warning("Failed to discover Jan models: %s", exc)
    ...
    def _parse_model(self, model_data: Dict[str, Any]) -> None:
        ...
        model_info = ModelInfo(
            ...
            runtime="jan",
            local=True,
            description=f"Jan model: {model_id}",
        )
    ...
    def generate(self, request: GenerateRequest) -> GenerateResponse:
        ...
        if not self.is_available():
            return GenerateResponse(
                ...
                success=False,
                error="Jan not available",
            )

        model_name = request.model.replace("jan:", "")
        start = time.time()
        ...
        try:
            ...
            logger.error("Jan generation failed: %s", exc)
            return GenerateResponse(
                content="",
                model=request.model,
                provider=self.provider_type,
    ...
    def stream(self, request: GenerateRequest) -> Iterator[StreamChunk]:
        if not self.is_available():
            yield StreamChunk(content="[Error] Jan not available", finish_reason="error")
            return
        ...
        try:
            ...
            logger.warning("Jan streaming failed: %s", exc)
